setwd("D:\\OneDrive\\Documents\\MCMC_2")
...
#
#   Database Used:  Annuity 2000 Basic Table. Minimum Age: 5 Maximum Age: 115
...
#     1.1.1     |   10/06/15    |   Zhiguo Wang |      work directory modified
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
headea_m <- read.csv("Inputs\\probabilities\\Healthy\\Annuity 2000 Basic Table - Male.csv", header = TRUE)

headea_f <- read.csv("Inputs\\probabilities\\Healthy\\Annuity 2000 Basic Table - Female.csv", header = TRUE)

headis_m <- read.csv("Inputs\\probabilities\\Healthy\\research-ltc-study-2000-11-agg-incidence-rates Male.csv", header = TRUE)

headis_f <- read.csv("Inputs\\probabilities\\Healthy\\research-ltc-study-2000-11-agg-incidence-rates Female.csv", header = TRUE)

dishea_m <- read.csv("Inputs\\probabilities\\Unhealthy\\2005 Group Term Life Waiver Study  Ultimate Probability of Recovery Following Disability Male.csv", header = TRUE)

dishea_f <- read.csv("Inputs\\probabilities\\Unhealthy\\2005 Group Term Life Waiver Study  Ultimate Probability of Recovery Following Disability Female.csv", header = TRUE)

disdea_m <- read.csv("Inputs\\probabilities\\Unhealthy\\PBGC Table Va Mortality Rates for Disabled Male.csv", header = TRUE)

disdea_f <- read.csv("Inputs\\probabilities\\Unhealthy\\PBGC Table Va Mortality Rates for Disabled Female.csv", header = TRUE)
...
ls()
length(ls())
save.image(file = "HealthStatusMatrix.RData")
